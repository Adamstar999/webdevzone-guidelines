```
# Recursos

Aqui dejo varios listados sobre todos los recursos usados ya sean videos, paginas web, contenido de otras personas como tal o contenido que haya hecho yo.
```

```
## YouTube
- **[Aprende VS Code ahora! | curso completo de VSCode desde CERO](https://www.youtube.com/watch?v=Ei1y51K8jQk)**
-   **[Cómo configurar VSCode para que sea ASOMBROSO! 😎🤓 | Extensiones, Tips y temas](https://www.youtube.com/watch?v=HiVnGgYudLY)**
-   **[Curso Markdown - jonmircha](https://www.youtube.com/watch?v=FlsoBiteuPM)**
- **[MEGA - Sincronizar Carpetas Con La NUBE / Almacenamiento En La Nube / Discos Virtuales](https://www.youtube.com/watch?v=Y9g3bnrYUuQ)**
-   **[¿Qué es una terminal? - Terminal en español - Terminal de Linux](https://terminaldelinux.com/terminal/introduccion/que-es-terminal/)**
-   **[Curso Terminal - jonmircha](https://www.youtube.com/watch?v=Pi0KVD4xTbc)**
-   **[Curso HTML & CSS (1/5): Fundamentos HTML - jonmircha](https://www.youtube.com/watch?v=-oK6zL01fNM)**
- **[Curso HTML & CSS ( 2 / 5 ): Fundamentos CSS - jonmircha](https://www.youtube.com/watch?v=K3xmRF8ab1o)**
- **[Curso HTML & CSS ( 3 / 5 ): Unidades y Estilos CSS - jonmircha](https://www.youtube.com/watch?v=p_lT7AgpEMU)**
- **[Curso HTML & CSS ( 4 / 5 ): Efectos Visuales y Movimiento en CSS - jonmircha](https://www.youtube.com/watch?v=mVhoGXkDbMw)**
- **[Curso HTML & CSS ( 5 / 5 ): Responsive Design y Arquitectura CSS - jonmircha](https://www.youtube.com/watch?v=udGrXWeJp1Y)**
- **[Qué es una CDN y para qué sirve una Red de distribución de contenidos](https://www.youtube.com/watch?v=_RSP3wagz7c)**
- **[How to test a local website on your phone](https://www.youtube.com/watch?v=uRYHX4EwYYA)**
- **[Curso Flexbox CSS](https://www.youtube.com/playlist?list=PLvq-jIkSeTUbQc3dGsssp8lxAi5npMrys)**
- **[Curso Grid CSS](https://www.youtube.com/playlist?list=PLvq-jIkSeTUY628cyd9LVbXSXi2xG9mUl)**
-   **[CSS Grid vs Flexbox - Cuando usar uno u otro.](https://www.youtube.com/watch?v=6qko7Nbe8YA)**
-   **[APRENDE a usar la METODOLOGÍA BEM](https://www.youtube.com/watch?v=6LUz0MnNCiA)**
-   **[APRENDE a usar AFTER y BEFORE en CSS](https://www.youtube.com/watch?v=VsYqbdBOP50)**
-   **[SocraTips - Cómo funcionan las CONTAINER QUERIES de CSS - SocraTech](https://www.youtube.com/watch?v=Mg6HS84QJ9A)**
-   **[¿Qué unidades de medida usar en CSS? 🤔](https://www.youtube.com/watch?v=EbsyJrtJgpw)**
- **[Print Styles: CSS Tutorial (Day 6 of CSS3 in 30 Days)](https://www.youtube.com/watch?v=CAgLAeykOyU)**
-   **[Guía de Emmet + Tips y Consejos para Visual Studio Code](https://www.youtube.com/watch?v=71XPXr7Tf-Y)**
- **[Curso JavaScript](https://www.youtube.com/playlist?list=PLvq-jIkSeTUZ6QgYYO3MwG9EMqC-KoLXA)**
-   **[¡Aprende los nuevos métodos de Array para JavaScript!](https://www.youtube.com/watch?v=TJKAGh9jzx4)**
-   **[¿Uso let o var? | #JavaScript](https://www.youtube.com/shorts/cFD9bB3UEtY)**
-   **[La MAGIA de la RECURSIVIDAD](https://www.youtube.com/watch?v=yX5kR63Dpdw)**
-   **[👨‍🏫 Resuelve Fibonacci con JavaScript: solución con recursividad y con bucles](https://www.youtube.com/watch?v=6ji-oYS6EgM)**
-   **[jQuery Tutorial Español - 💪 Curso de jQuery 2018 desde cero 💪](https://www.youtube.com/watch?v=DVN8NWppCN0)**
-   **[GitHub vs GitLab | PlatziLive](https://www.youtube.com/watch?v=EscDe0jG6XM)**
-   **[Curso Git & GitHub - jonmircha](https://www.youtube.com/watch?v=suzMNqDQiyU)**
-   **[Aprende Git y GitLab desde cero con este curso para principiantes, tutorial paso a paso 2023](https://www.youtube.com/watch?v=6NREQqA5wHE)**
- **[⚛️ REACT 2023: Todas las opciones para comenzar - jonmircha](https://www.youtube.com/watch?v=lH96nhwaMWY)**
- **[Curso de Webpack - jonmircha](https://www.youtube.com/watch?v=-bp3q-YTr4Q)**
```

```
## Sitios Web

-   **[RoadMap](https://roadmap.sh/frontend)**
-   **[FreeCodeCamp](https://www.freecodecamp.org/learn)**
-   **[Lighthouse de Google](https://developer.chrome.com/docs/lighthouse/)**
-   **[DevDocs](https://devdocs.io)**
-   **[WebDev](https://web.dev/learn/)**
-   **[Página web de JonMircha](https://jonmircha.com/cursos)**
-   **[Live Server](https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer)**
-   **[Auto Rename Tag](https://marketplace.visualstudio.com/items?itemName=formulahendry.auto-rename-tag)**
-   **[Error Lens](https://marketplace.visualstudio.com/items?itemName=usernamehw.errorlens)**
-   **[Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode)**
-   **[Markdown 🦊 jonmircha](https://jonmircha.com/markdown)**
-   **[markdown](https://joedicastro.com/pages/markdown.html)**
-   \*\*[ChatGPT](https://chat.openai.com/auth/login)
-   **[SVG Repo](https://www.svgrepo.com/)**
-   **[Periodic Table of HTML elements](https://madebymike.github.io/html5-periodic-table/)**
-   **[cdnjs](https://cdnjs.com/)**
-   **[Code Guide by @mdo](https://codeguide.co)**
-   **[30 HTML Best Practices for Beginners](https://webdesign.tutsplus.com/30-html-best-practices-for-beginners--net-4957t)**
- **[Fast Youtube Embed Generator](https://tube.rvere.com)**
- **[¿Qué imágenes debes usar en tu web? ¿JPG, PNG, SVG o WEBP? | EDteam](https://ed.team/blog/que-imagenes-debes-usar-en-tu-web-jpg-png-svg-o-webp)**
-   **[Patrones de gradientes CSS](https://bennettfeely.com/gradients/)**
-   **[Generador de textos en 3D](https://bennettfeely.com/ztext/)**
-   **[Manipulador de FlexBox](https://bennettfeely.com/flexplorer/)**
-   **[Generador de gráficos circulares](https://bennettfeely.com/csspiechart/)**
-   **[Clip de imágenes](https://bennettfeely.com/clippy/)**
-   **[Filtros para imágenes en CSS](https://bennettfeely.com/image-effects/)**
- **[App - CSS Icons](https://css.gg/app)**
- **[Type Scale - A Visual Calculator](https://typescale.com)**
- **[Blog | Utopia](https://utopia.fyi/blog/)**
- **[Tools for designers & frontend developers](https://www.magicpattern.design/tools)**
- **[Autoprefixer CSS online](https://autoprefixer.github.io)**
- **[CSS Minifier](https://www.tutorialsteacher.com/tools/css-minifier)**
- **[PXtoEM.com: PX to EM conversion made simple.](http://pxtoem.com)**
- **[hue.tools – info](https://hue.tools/info)**
- **[CSS Gradient — Generator, Maker, and Background](https://cssgradient.io)**
- **[Google Fonts](https://fonts.google.com)**
- **[Media Queries](https://mediaqueri.es)**
- **[Josh W Comeau](https://www.joshwcomeau.com)**
- **[airbnb/css: A mostly reasonable approach to CSS and Sass.](https://github.com/airbnb/css)**
- **[necolas/idiomatic-css: Principles of writing consistent, idiomatic CSS.](https://github.com/necolas/idiomatic-css)**
- **[CSS Guidelines (2.2.5)](https://cssguidelin.es)**
- **[Thank you for downloading Quick HTML Color Picker from CNET Download.com](https://download.cnet.com/quick-html-color-picker/3001-2192_4-10400979.html)**
- **[CSS Overview: Identify potential CSS improvements](https://developer.chrome.com/docs/devtools/css-overview/)**
- **[scroll-margin-top - CSS: Cascading Style Sheets | MDN](https://developer.mozilla.org/en-US/docs/Web/CSS/scroll-margin-top)**
- **[Atributos: “@import vs link” – Aprendiendo CSS](http://www.loretahur.net/AprendiendoCSS/atributos-“import-vs-link”/)**
- **[CSS property order | @mdo](https://markdotto.com/2011/11/29/css-property-order/)**
- **[Emmet Cheat Sheet](https://docs.emmet.io/cheat-sheet/)**
- **[24 buenas prácticas de JavaScript para principiantes](https://code.tutsplus.com/es/24-javascript-best-practices-for-beginners--net-5399t)**
- **[airbnb/javascript: JavaScript Style Guide](https://github.com/airbnb/javascript)**
- **[Deep Copying JavaScript Arrays](https://medium.com/@ziyoshams/deep-copying-javascript-arrays-4d5fc45a6e3e)**
-   **[Hoja de referencia de sintaxis de expresiones regulares](https://developer.mozilla.org/es/docs/Web/JavaScript/Guide/Regular_expressions/Cheatsheet)**
- **[Documentación oficial de jQuery](https://api.jquery.com)**
- **[Introducción a Git por Jonmircha](https://jonmircha.com/git)**
```

```
## Ejercicios y Proyectos

- **[Learn HTML by Building a Cat Photo App - FreeCodeCamp](https://www.freecodecamp.org/learn/2022/responsive-web-design/#learn-html-by-building-a-cat-photo-app)**
-   **[💻 COMO crear una PAGINA WEB en HTML y CSS [PASO A PASO] Adaptable a DISPOSITIVOS MÓVILES](https://www.youtube.com/watch?v=HH_SMpxV7qQ)**
-   **[💻 COMO crear una PAGINA WEB en HTML y CSS [PASO A PASO] Adaptable a DISPOSITIVOS MÓVILES #2](https://www.youtube.com/watch?v=1Lv8mJczfpg)**
-   **[Peaky blinders-page con Html y Css](www.youtube.com/watch?v=ArhSODreNHc)**
-   **[Responsive Web Design - FreeCodeCamp](https://www.freecodecamp.org/learn/2022/responsive-web-design/)**
-   **[Ejercicios con expresiones regulares - FreeCodeCamp](https://www.freecodecamp.org/learn/javascript-algorithms-and-data-structures/#regular-expressions)**
- **[Exercism](https://exercism.org/tracks/javascript/exercises)**
- **[JavaScript Algorithms and Data Structures - FreeCodeCamp](https://www.freecodecamp.org/learn/javascript-algorithms-and-data-structures/)**
- **[JavaScript Mastery](https://www.youtube.com/@javascriptmastery)**
- **[FreeCodeCamp](https://www.freecodecamp.org/learn/front-end-development-libraries/)**
-   **[Curso Bootstrap - jonmircha (Studio Bad Dog)](www.youtube.com/watch?v=ng8ur8KNrpU&t=11114s)**
-   **[Crea una Página WEB con BOOTSTRAP 5 desde 0 | Rápido](https://www.youtube.com/watch?v=A5MF1mwmgRI)**
-   **[Nintendo, Sección de Animal Crossing](https://www.youtube.com/watch?v=ZuOL_DoaG9k)**
```

```
## Consejos, Trucos y Buenas Prácticas
```

```
## Extra

-   **[Guía de la Terminal](./assets/Bash-es.md)**

- **[Estructura de Proyectos con React](./assets/React%20SRC.jpg)**
```
